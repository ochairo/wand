...
func (h *ListCommandHandler) Handle(ctx interfaces.CommandContext) error {
	...
	// Check flags
	remoteFlag, _ := ctx.GetBoolFlag("remote")

	// If no package name provided, list all installed packages
	if len(args) == 0 {
		if remoteFlag {
			return fmt.Errorf("package name required when using --remote flag")
		}
		return h.listAllPackages(ctx)
	}

	packageName := args[0]

	if remoteFlag {
		// Fetch available versions from potions repository
		versions, err := h.versionService.ListAvailableVersions(packageName)
		if err != nil {
			return fmt.Errorf("failed to fetch remote versions: %w", err)
		}

		if len(versions) == 0 {
			ctx.Printf("No versions available for '%s'\n", packageName)
			return nil
		}

		ctx.Printf("Available versions of %s (%d):\n", packageName, len(versions))
		for _, version := range versions {
			ctx.Printf("  %s\n", version.String())
		}
		ctx.Printf("\nInstall with: wand install %s@<version>\n", packageName)
		return nil
	}

	// Local listing for specific package
	return h.listPackageVersions(ctx, packageName)
}

// listAllPackages lists all installed packages
func (h *ListCommandHandler) listAllPackages(ctx interfaces.CommandContext) error {
	...
	if len(registry.Packages) == 0 {
		...
		ctx.Printf("\nInstall packages with: wand install <package>\n")
		return nil
	}

	ctx.Printf("Installed packages:\n")
	for packageName, entry := range registry.Packages {
		globalVersion := registry.GlobalVersions[packageName]
		versionCount := len(entry.Versions)
		if versionCount == 1 {
			ctx.Printf("  %s (%s)\n", packageName, globalVersion)
		} else {
			ctx.Printf("  %s (%d versions, active: %s)\n", packageName, versionCount, globalVersion)
		}
	}

	ctx.Printf("\nUse 'wand list <package>' to see all versions of a specific package\n")
	return nil
}

// listPackageVersions lists versions of a specific package
func (h *ListCommandHandler) listPackageVersions(ctx interfaces.CommandContext, packageName string) error {
	registry, err := h.registryRepo.Load()
	if err != nil {
		return fmt.Errorf("failed to load registry: %w", err)
	}
	...
	if !exists {
		...
		ctx.Printf("\nInstall it first: wand install %s\n", packageName)
		return nil
	}
	...
	for version, pkg := range entry.Versions {
		...
		ctx.Printf("  %s %s (installed %s)\n", marker, version, pkg.InstalledAt.Format("2006-01-02"))
	}

	return nil
}

// UseCommandHandler handles the use command (deprecated, use SwitchCommandHandler)
type UseCommandHandler struct {
	registryRepo interfaces.RegistryRepository
	wandrcRepo   interfaces.WandRCRepository
}

// NewUseCommandHandler creates a new use command handler (deprecated)
...
// Handle executes the use command (deprecated)
...
// SwitchCommandHandler handles the switch command
type SwitchCommandHandler struct {
	registryRepo interfaces.RegistryRepository
	wandrcRepo   interfaces.WandRCRepository
}

// NewSwitchCommandHandler creates a new switch command handler
func NewSwitchCommandHandler(
	registryRepo interfaces.RegistryRepository,
	wandrcRepo interfaces.WandRCRepository,
...
// Handle executes the switch command
func (h *SwitchCommandHandler) Handle(ctx interfaces.CommandContext) error {
	args := ctx.GetArgs()
	if len(args) == 0 {
		return fmt.Errorf("usage: wand switch <package>@<version>")
	}
	...
	parts := strings.Split(packageSpec, "@")
	if len(parts) != 2 {
		return fmt.Errorf("version required (use package@version format, e.g., kubectl@1.30.0)")
	}
	...
	global, err := ctx.GetBoolFlag("global")
	if err != nil {
		global = false // default to local/project scope
	}
	...
	if _, exists := entry.Versions[version.String()]; !exists {
		return fmt.Errorf("version %s of package '%s' is not installed\n\nInstall it first: wand install %s@%s", version, packageName, packageName, version)
	}
	...
	if global {
		...
		ctx.Printf("✓ Switched global version of %s to %s\n", packageName, version)
	} else {
		// Set project version in .wandrc
		wandrc, err := h.wandrcRepo.Load(".")
		if err != nil {
		...
		ctx.Printf("✓ Switched project version of %s to %s\n", packageName, version)
	}

	return nil
}

// InfoCommandHandler handles the info command
type InfoCommandHandler struct {
	registryRepo interfaces.RegistryRepository
	wandrcRepo   interfaces.WandRCRepository
}

// NewInfoCommandHandler creates a new info command handler
func NewInfoCommandHandler(
	registryRepo interfaces.RegistryRepository,
	wandrcRepo interfaces.WandRCRepository,
) *InfoCommandHandler {
...
func (h *InfoCommandHandler) Handle(ctx interfaces.CommandContext) error {
	args := ctx.GetArgs()
	if len(args) == 0 {
		return fmt.Errorf("package name required")
	}
	...
	// Global version
	globalVersion, hasGlobal := registry.GetGlobalVersion(packageName)
	if hasGlobal {
		ctx.Printf("Active globally: %s\n", globalVersion)
	} else {
		ctx.Printf("Active globally: none (using first installed version)\n")
	}

	// Project version (from .wandrc)
	if h.wandrcRepo.Exists(".") {
		wandrc, err := h.wandrcRepo.Load(".")
		if err == nil && wandrc.HasVersion(packageName) {
			projectVersion, _ := wandrc.GetVersion(packageName)
			ctx.Printf("Active in project: %s (from .wandrc)\n", projectVersion)
		} else {
			ctx.Printf("Active in project: (using global version)\n")
		}
	}
	...
	for version, pkg := range entry.Versions {
		...
		ctx.Printf("  %s %s\n", marker, version)
		ctx.Printf("    Install path: %s\n", pkg.InstallPath)
		ctx.Printf("    Installed: %s\n", pkg.InstalledAt.Format("2006-01-02 15:04:05"))
	}
