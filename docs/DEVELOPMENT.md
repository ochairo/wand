...
domain/services             │ (TUI/Web/etc)
    ↓ uses                  │     ↓
domain/entities             │ pkg/client (Public API)
    ↑                       │     ↓ converts
    │ implements            │ pkg/types (Public DTOs)
    │                       │     ↑
domain-adapters ────────────┘     │
    ↓ uses                        │
external-adapters ────────────────┘
...
- `internal/` cannot be imported by external code (Go rule)
- `pkg/client` bridges internal implementation to public API
- `pkg/types` are simplified DTOs (Data Transfer Objects) for external use
- Conversion happens at the `pkg/client` boundary
- Internal code continues using `entities.*` types
- External tools only see `types.*` types
...
├── pkg/                       # Public API (importable by external tools)
│   ├── client/                # High-level client for third-party use
│   ├── types/                 # Public DTOs (Package, Formula, Version, etc.)
...
├── examples/
│   └── api-usage/             # Example programs using pkg/client
├── docs/                      # Documentation
